...
> I just ran aws s3 cp the the part to the directory,
> the bucket name then added --recursive
...
> use aws cp but include the actual path to the directory
...
> sync allows you to keep track of updates and changes over time.
...
> aws-vault exec jake -- aws s3api put-object-acl
> --bucket stelliget-u-jake.awe.labs --key private.txt
> --acl public-read
> i simply added --acl public-read
...
> You can use same  put-object-acl command as above
...
> The default is Private to only the owner
...
> YES aws s3api get-object --bucket bucket
> --version-id version-id --key key output

##### Question: Deleting All Versions

_How would you delete all versions?_
> You need to list all versions then iterate over them with a delete operation
...
> Yes using cloudformation you can update tags individually
...
> No 30 is the min for IA and 90 is the min for GLACIER
...
> Yes you can use this for app data
...
> KMS give user access control with audit trails and logs of use
...
> Better control, you can create, rotate, and disable CMKs in KMS
