...
cloudformation_tail() {
  set -e
  local stack="jake-auto-scalings"
  local stackId
  ...
  if [ -z "$stackStatus" ] ; then 
    ...
    stackId=$(aws-vault exec jake -- aws cloudformation create-stack --stack-name $stack --template-body file://template.yaml --parameters file://parameters.json --capabilities CAPABILITY_IAM | jq -c -r .StackId)
    ...
    set +e
    stackId=$(aws-vault exec jake -- aws cloudformation update-stack --stack-name $stack --template-body file://template.yaml --parameters file://parameters.json --capabilities CAPABILITY_IAM | jq -c -r .StackId  2>&1)
    status=$?
